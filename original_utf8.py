...
class FiltroEventosDicom(QObject):
    ...
    enquanto consome o scroll da roda do mouse para avan├ºo de fatias e Ctrl+Left para roll 2D.
    """
    ...
    def _hit_test(self, pos):
        ...
        # Se a proje├º├úo estiver em modo Normal, ignora completamente as linhas
                ...
                if event.type() == QEvent.Type.Wheel:
                    try:
                        ...
                        if delta != 0:
                            ...
                            # Obt├⌐m o plano correspondente ou usa o primeiro dispon├¡vel (fallback seguro)
                            ...
                            if plane and nav.volume_ativo:
                                ...
                                # Atualiza as linhas do MIP apenas se todos os planos padr├úo existirem no navegador
                                ...
                                # Busca segura pela MainWindow para extrair as vis├╡es ativas na tela agora
                                win = self.parent()
                                ...
                                # --- SINCRONIA INTERNA DOS PLANOS-IRM├âOS ---
                                if sync_active:
                                    # Se a sincronia estiver ativa, empurra os planos das OUTRAS telas vis├¡veis deste mesmo coordenador
                                ...
                                # Notifica APENAS as fatias que est├úo fisicamente vis├¡veis no layout ativo
                                for visao_nome, rnd in nav.renderers_2d.items():
                                    if visao_nome in visoes_visiveis:  # Filtro r├¡gido
                                ...
                                # Sincroniza├º├úo de rolagem global (Sync Scroll)
                    ...
                    # Rastreamento de Foco (Last Clicked Viewport)
                    win = self.parent()
                    while win is not None and not hasattr(win, "coordenador_exibicao"):
                        win = win.parent() if hasattr(win, "parent") else None
                    ...
                    if win and hasattr(win, "coordenador_exibicao") and win.coordenador_exibicao.widget_layout_ativo:
                        ...
                        if self.nome_visao in visoes:
                            quadrante = visoes[self.nome_visao]
                            win.active_quadrante = quadrante
                    ...
                    if event.button() == Qt.MouseButton.LeftButton:
                        if self.ferramenta_ativa == "Semente":
                            ...
                            if renderer:
                                ...
                                # Convers├úo matem├ítica imaculada VTK -> Index (Voxel)
                                idx = [0.0, 0.0, 0.0]
                        ...
                        if self.ferramenta_ativa in ["Regua", "Elipse", "SementeDSA"]:
                            ...
                            if renderer:
                                ...
                                # --- L├ôGICA DA SEMENTE DSA AQUI ---
                                if self.ferramenta_ativa == "SementeDSA":
                                    main_window = self.parent().parent() if hasattr(self.parent(), 'parent') else None
                                    
                                    # Usa o volume_ativo diretamente do navegador_2d que j├í est├í neste escopo
                                    vtk_img = self.navegador_2d.volume_ativo
                                    
                                    if main_window and vtk_img:
                                        try:
                                            # Converte a coordenada 3D f├¡sica diretamente para o ├ìndice Voxel da Matriz
                                            idx = [0.0, 0.0, 0.0]
                            ...
                            # Coordenadas f├¡sicas exatas para o VTK
                            x_vtk = event.position().x() * dpr
                            # Invers├úo do Y considerando a altura l├│gica multiplicada pelo DPR
                            y_vtk = (self.interactor.height() - event.position().y()) * dpr
                        ...
                        if self.ferramenta_ativa == "Reslice":
                            ...
                            if renderer:
                                ...
                                if self.alvo_reslice is not None:
                                    ...
                                    return True
                            # Clique no vazio em modo Reslice ΓåÆ janelamento normal
                            self.ultimo_pos_x = event.position().x()
                            self.ultimo_pos_y = event.position().y()
                            self.arrastando_janelamento = True
                            return True
    
                        # L├│gica de arrasto de linhas (Navega├º├úo Interativa)
                        ...
                        if self.ferramenta_ativa == "Normal" and hasattr(self.parent(), 'coordenador_medidas') and self.parent().coordenador_medidas.medidas:
                            ...
                            if renderer:
                                ...
                                if medida_acertada:
                                    ...
                                    # Captura segura da posi├º├úo de mundo para calcular deltas
                                    cell_picker = vtk.vtkCellPicker()
                                    cell_picker.Pick(x_vtk, y_vtk, 0.0, renderer)
                                    self.ultima_posicao_medida = cell_picker.GetPickPosition()
                        ...
                        if event.modifiers() & Qt.KeyboardModifier.ControlModifier:
                            ...
                            return True  # Consome para rota├º├úo 2D
                        else:
                            self.arrastando_janelamento = True
                            return True  # Consome para janelamento manual
                    # Deixa cliques normais flu├¡rem para a engine do VTK
                    return False
                    
                elif event.type() == QEvent.Type.MouseButtonDblClick:
                    # Impede que o duplo clique ative fun├º├╡es nativas do C++ do VTK 
                    # (como window/level nativo) que poderiam ficar com o mouse preso
                    return True
                    
                elif event.type() == QEvent.Type.MouseMove:
                    if event.buttons() == Qt.MouseButton.NoButton:
                        # Fallback robusto: se o mouse mover sem bot├╡es apertados, destrava
                        self.arrastando_janelamento = False
                        self.arrastando_rotacao = False
                        self.arrastando_crosshair = False
                        self.alvo_reslice = None
                        if getattr(self, 'alvo_arraste', None):
                            self.alvo_arraste = None
                            self.interactor.setCursor(Qt.CursorShape.ArrowCursor)
                    ...
                    if event.buttons() == Qt.MouseButton.NoButton:
                        ...
                        if renderer:
                            ...
                            if camera:
                                camera.Roll(dx * 0.5)
                                # Atualiza a b├║ssola anat├┤mica ap├│s a rota├º├úo
                    ...
                    # Pega a tecla 'C' (ignorando a repeti├º├úo autom├ítica do teclado)
...
class FiltroEventosDicom3D(QObject):
    """
    Filtro de eventos PyQt6 para a visualiza├º├úo 3D.
    Intercepta Shift + Clique Esquerdo para ajustar o janelamento 3D,
    deixando rota├º├úo nativa (Left Click comum), Pan (Middle) e Zoom (Right) flu├¡rem.
    """
            ...
            if obj == self.interactor:
                if event.type() == QEvent.Type.MouseButtonPress:
                    ...
                    if win and hasattr(win, "coordenador_exibicao") and win.coordenador_exibicao.widget_layout_ativo:
                        visoes = win.coordenador_exibicao.widget_layout_ativo.visoes
                        if "3D" in visoes:
                            win.active_quadrante = visoes["3D"]
                    ...
                    if event.button() == Qt.MouseButton.LeftButton:
                        ...
                        if event.modifiers() & Qt.KeyboardModifier.ShiftModifier:
                            ...
                            self.ultimo_pos_x = event.position().x()
                            self.ultimo_pos_y = event.position().y()
                            return True  # Consome para evitar rota├º├úo nativa
                    ...
                    if event.buttons() == Qt.MouseButton.NoButton:
                        # Fallback robusto: solta todas as travas se o mouse se mover sem bot├╡es pressionados
                        self.arrastando_janelamento = False
                        self.arrastando_rotacao = False
                        self.arrastando_crosshair = False
                        self.alvo_reslice = None
                    ...
                    if event.button() == Qt.MouseButton.LeftButton:
                        if getattr(self, 'arrastando_bisturi', False):
                            ...
                            if len(self.bisturi_pontos) > 2:
                                # Repete o primeiro ponto no final para fechar o la├ºo visualmente
                            ...
                            # Armazena os dados no coordenador pai para que os bot├╡es do menu possam acess├í-los
